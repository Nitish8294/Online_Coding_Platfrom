...
import { loginUser, loginStart, loginSuccess, loginFailure } from "../authSlice";
      ...
      <div className="card w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl relative z-10">
        <div className="card-body p-8 sm:p-10">
          <div className="text-center mb-8">
            <h2 className="text-4xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400 mb-2">
              LeetCode
            </h2>
            ...
            <div className="form-control">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`input input-bordered w-full bg-slate-950/50 border-slate-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 pl-11 rounded-xl transition-all ${errors.emailId ? 'input-error' : ''}`}
                  {...register('emailId')}
                />
              </div>
              {errors.emailId && (
            ...
            <div className="form-control">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`input input-bordered w-full bg-slate-950/50 border-slate-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 pl-11 pr-12 rounded-xl transition-all ${errors.password ? 'input-error' : ''}`}
                  {...register('password')}
                />
                <button
                  type="button"
              ...
              <label className="label">
                <span className="label-text-alt link link-hover text-slate-500 hover:text-indigo-400 transition-colors">Forgot password?</span>
              </label>
            </div>

            <button
              type="submit"
              className={`btn w-full bg-gradient-to-r from-indigo-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 text-white border-none shadow-lg shadow-indigo-500/20 rounded-xl h-12 text-base font-bold tracking-wide mt-2 ${loading ? 'opacity-70 cursor-not-allowed' : 'transform active:scale-[0.98] transition-all'}`}
              disabled={loading}
            >
              {loading ? (
